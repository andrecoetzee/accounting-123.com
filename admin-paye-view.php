...
";
	...
	if (pg_numrows ($payeRslt) > 0) {
		while ($myPaye = pg_fetch_array ($payeRslt)) {
			$showPaye .= "<tr class='".bg_class()."'><td align=right>".CUR." $myPaye[min]</td><td align=right>".CUR." $myPaye[max]</td><td align=right>$myPaye[percentage]%</td><td><a href='admin-paye-edit.php?id=$myPaye[id]'>Edit</a> | <a href='admin-paye-rem.php?id=$myPaye[id]'>Delete</a></td></tr>\n";
			$i++;
		}
	} else {
