...
function details($HTTP_POST_VARS, $error="")
{
	...
	if (isset($letters)) {
		...
		if (pg_numrows ($custRslt) < 1) {
			...
			db_connect();
			# Query server for customer info
			$sql = "SELECT cusnum,cusname,surname,accno FROM customers WHERE deptid = '$inv[deptid]' AND location != 'int' AND lower(surname) LIKE lower('$letters%') AND blocked != 'yes' AND div = '".USER_DIV."' ORDER BY surname";
			$custRslt = db_exec ($sql) or errDie ("Unable to view customers");
			if (pg_numrows ($custRslt) < 1) {
				$ajax_err = "<li class='err'>No customer names starting with <b>$letters</b> in database.</li>";
				//return view_err($HTTP_POST_VARS, $err);
...
//								$Wh = "AND (lower(substr(stkdes,1,'$len'))=lower('$ria') OR lower(substr(stkcod,1,'$len'))=lower('$ria'))";
					...
					//		$cods .= "<option value='-S' disabled selected>Select Number</option>";
							...
							while($stk = pg_fetch_array($stkRslt)){
								...
								$cods .= "
								<option value='$stk[stkid]'>
									$stk[stkcod] (".($stk['units'] - $stk['alloc']).")
								</option>";
							}
							$cods .= "</select> ";
...
//					$cods .= "<option value='-S' disabled selected>Select Number</option>";
					...
					$descs = "<select name='stkidss[$ai]' onChange='javascript:document.form.submit();'>";
					$descs .= "<option value='-S' disabled selected>Select Description</option>";
					$count = 0;
					while($stk = pg_fetch_array($stkRslt)){
						// Check if this stock item has been blocked
...
/*--- Start checks --- */
	...
	#get a valid vatcode for delivery charge
	...
	$details_end = "
					...
					<td valign='top' align='right' width='50%'>
						<table ".TMPL_tblDflts.">
							...
							<input type='hidden' name='chrgvat' value='inc'>
							<tr bgcolor='".bgcolorg()."'>
								<td>VAT Inclusive</td>
							...
							<input type='hidden' name='traddisc' value='0'>
							<input type='hidden' name='delchrg' value='0'>
							<input type='hidden' name='delvat' value='$delvat'>
							<input type='hidden' name='del_date_year' value='$inv_date_year'>
							<input type='hidden' name='del_date_month' value='$inv_date_month'>
							<input type='hidden' name='del_date_day' value='$inv_date_day'>
						</table>
					</td>
				</tr>
				".TBL_BR."
				...
				if(isset($remprod)&&in_array($keys, $remprod)){
					...
					$sql = "
						INSERT INTO inv_items (
							invid, whid, stkid, qty, 
							unitcost, amt, disc, discp, 
							ss, serno, div, vatcode, del
						) VALUES (
							'$invid', '$whids[$keys]', '$stkids[$keys]', '$qtys[$keys]',
							'$unitcost[$keys]', '$amt[$keys]', '$disc[$keys]', '$discp[$keys]',
							'$sernos_ss[$keys]', '$sernos[$keys]', '".USER_DIV."', '$vatcodes[$keys]','0'
						)";
					$rslt = db_exec($sql) or errDie("Unable to insert invoice items to Cubit.",SELF);
...
/* --- ----------- Clac --------------------- */
		...
		# insert invoice to DB
		$sql = "UPDATE invoices SET delvat='$delvat',cusnum = '$cusnum', deptname = '$dept[deptname]', cusacc = '$cust[accno]', cusname = '$cust[cusname]', surname = '$cust[surname]', cusaddr = '$cust[addr1]', cusvatno = '$cust[vatnum]', cordno = '$cordno', ordno = '$ordno', docref = '$docref',
		chrgvat = '$chrgvat', terms = '$terms', salespn = '$salespn', odate = '$odate', traddisc = '$traddisc', delchrg = '$delchrg', subtot = '$SUBTOT', vat = '$VAT', total = '$TOTAL', balance = '$TOTAL', comm = '$comm', serd = 'y', discount='$traddiscmt', delivery='$delexvat', branch = '$branch', deldate = '$deldate',
		bankid = '$bankid' WHERE invid = '$invid'";
