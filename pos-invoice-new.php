...
function details($_POST, $error="")
{
	...
	if (isset($prev_cusnum) && $prev_cusnum != $cusnum) {
		...
		#also set the sales person ...
		$get_salespn = "SELECT salesp FROM salespeople WHERE salespid = '$cust[sales_rep]' LIMIT 1";
		$run_salespn = db_exec ($get_salespn) or errDie ("Unable to get sales person information.");
		if (pg_numrows ($run_salespn) > 0){
			#found !
			$inv['salespn'] = pg_fetch_result ($run_salespn,0,0);
		}else {
			$inv['salespn'] = "0";
		}
	}
...
//fixes broken store change function ...
...
/* --- Start Products Display --- */
	...
	if(isset($whidss)){
		foreach($whidss as $key => $whid){
			if(isset($stkidss[$key]) && $stkidss[$key] != "-S" && (strlen($stkidss[$key]) > 0)){
				...
				db_conn("exten");
				$sql = "SELECT whid, whname FROM warehouses WHERE whid = '$stk[whid]' AND div = '".USER_DIV."'";
				$whRslt = db_exec($sql);
				$wh = pg_fetch_array($whRslt);
				...
				$products .= "
					...
					<input type='hidden' name='descriptions[]' value=''>
					<tr bgcolor='".bgcolorg()."'>
						<td><input type='hidden' name='whids[]' value='$wh[whid]'>$wh[whname]</td>
						<td>
				...
				if(!isset($addnon)) {
					...
					if (pg_numrows ($stkRslt) < 1) {
						...
						if (isset($sel_frm) && $sel_frm == "stkdes") {
							$descs = "<select style='width:250px' name='stkidss[]' onChange=\"document.form.des.value=''; javascript:document.form.submit();\">";
							...
							$cods = "<select style='width:250px' name='stkidss[]' onChange=\"document.form.des.value=''; javascript:document.form.submit();\">";
	//						$cods .= "<option value='-S' disabled selected>Select Number</option>";
		...
		if(!isset($addnon) && !isset($upBtn)){
			...
			if (pg_numrows($stkRslt) == 1){
				$reload = TRUE;
				$send_override = "<input type='hidden' name='stkidss[]' value='".pg_fetch_result ($stkRslt,0,0)."'>";
			}else {
				$reload = FALSE;
				$send_override = "";
			}
			...
			if (isset($sel_frm) && $sel_frm == "stkdes") {
				...
				$stks = "<select style='width:250px' name='stkidss[]' onChange=\"document.form.des.value=''; javascript:document.form.submit();\">";
				$count = 0;
				while($stk = pg_fetch_array($stkRslt)){
					// Check if this stock item has been blocked
					if (stock_is_blocked($stk["stkid"])) {
...
/* --- End Some calculations --- */
	...
	if (isset ($addprodBtn) OR isset ($addnon) OR isset ($saveBtn) OR isset ($upBtn) OR isset ($des)){
		$jump_bot = "
			<script>
...
/* -- Final Layout -- */
	...
	<center>
	<h3>New Point of Sale Invoice</h3>
...
function write($_POST)
{
	...
	if(isset($qtys)){
		foreach($qtys as $keys => $qty){
			...
			if($qty < 1){
				$v->isOk ($qty, "float", 0, 0, "Error : Item Quantity must be at least one. Product number : <b>".($keys+1)."</b>");
			}
		}
	}
	# check whids
	...
	$rslt = db_exec($sql) or errDie("Unable to update invoice items in Cubit.",SELF);
		...
		if(isset($qtys)){
			foreach($qtys as $keys => $value){
				...
				if (isset ($sernos_ss[$keys]) AND $sernos_ss[$keys] == "*_*_*CUBIT_SERIAL_SELECT_BOX*_*_*") {
				...
				if(isset($remprod) && in_array($keys, $remprod)){
					...
					$sql = "
						INSERT INTO pinv_items (
							invid, whid, stkid, qty, 
							unitcost, amt, disc, discp, 
							ss, serno, div, vatcode, 
							description, account
						) VALUES (
							'$invid', '$whids[$keys]', '$stkids[$keys]', '$qtys[$keys]', 
							'$unitcost[$keys]', '$amt[$keys]', '$disc[$keys]', '$discp[$keys]', 
					...
					$sql = "
						INSERT INTO pinv_items (
							...
							'$invid', '$whids[$keys]', '$stkids[$keys]', '$qtys[$keys]', 
							'$unitcost[$keys]', '$amt[$keys]', '$disc[$keys]', '$discp[$keys]', 
							'$sernos_ss[$keys]', '$sernos[$keys]', '".USER_DIV."','$vatcodes[$keys]'
						)";
...
// 			$VAT=sprint(($subtotal-$taxex)*$VATP/(100+$VATP));
		...
		if($cusnum != "0"){
			...
			$cusacc = "";
			if (isset($client) AND strlen ($client) > 0){
				$cusname = $client;
			}else {
				$cusname = "";
			}
...
//cusname = '$client'
		...
		$sql = "
			UPDATE pinvoices 
			SET pcredit='$pcredit', cusnum='$cusnum', delvat='$delvat', rounding='$rounding', 
				pcash='$pcash', pcheque='$pcheque', pcc='$pcc', deptid='$deptid', 
				deptname = '$dept[deptname]', cusname = '$cusname', cordno = '$cordno', 
				ordno = '$ordno', chrgvat = '$chrgvat', salespn = '$salespn', odate = '$odate', 
				traddisc = '$traddisc', delchrg = '$delchrg', subtot = '$SUBTOT', vat = '$VAT', 
				total = '$TOTAL', balance = '$pcredit', comm = '$comm', discount='$traddiscmt', 
				delivery='$delexvat', vatnum='$vatnum', cusacc = '$cusacc', telno='$telno' 
	...
	if (strlen($bar) > 0) {
		...
		$sql = "
			INSERT INTO pinv_items (
				...
				amt, disc, discp, ss, serno, div
			) VALUES (
				'$invid', '$s[whid]', '$s[stkid]', '1', '$s[selamt]', 
...
/* --- Start button Listeners --- */
	if(isset($doneBtn)){
		...
		$OUTPUT = "
					...
					<input type='button' value='Create New POS Invoice' onClick=\"move('pos-invoice-new.php');\">";
		require("template.php");
...
function recvpayment_write()
{
			...
			if($out > 0) {
				$sql = "
					INSERT INTO cubit.open_stmnt (
						cusnum, invid, amount, balance, 
						date, type, st, div
					) VALUES (
						'$cus[cusnum]', '0', '-$out', '-$out', 
						'$sdate', 'Payment Received', 'n', '".USER_DIV."'
					)";
				$stmntRslt = db_exec($sql) or errDie("Unable to Insert statement record in Cubit.",SELF);
	...
	$sql = "
		INSERT INTO cubit.payrec (
			date, by, multiinv, amount, method, prd, note
		) VALUES (
			'$sdate', '".USER_NAME."', '$invidsers', '$pcash','Cash','".PRD_DB."','0'
		)";
	...
	$sql = "
		INSERT INTO cubit.payrec (
			...
			'$sdate', '".USER_NAME."', '$invidsers', '$pcc', 'Credit Card', '".PRD_DB."', '0'
		)";
	db_exec($sql) or errDie("Unable to insert data.");
	...
	$sql = "
		INSERT INTO cubit.payrec (
			...
			'$sdate', '".USER_NAME."', '$invidsers', '$pcheque', 'Cheque', '".PRD_DB."', '0'
		)";
	db_exec($sql) or errDie("Unable to insert data.");
