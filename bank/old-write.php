...
function write($HTTP_POST_VARS)
{
	...
	$save_sql = "
				INSERT INTO supp_payment_print 
					(supid, account, pay_date, sdate, refno, cheqno, total_amt, set_amt, overpay_amt, descript) 
				VALUES 
					('$supid','$bankid','$accdate','now','$reference','$cheqnum','$amt','$setamt','$overpay','$descript')";
				...
				foreach($invids as $key => $value)
				{
					...
					if($stock_setamt[$key] > 0){
						$sql = "
								...
								VALUES 
									('$sup[supid]','$stock_setamt[$key]','$sdate', 'Settlement Discount for Invoice (Ref. $reference): $invids[$key]','$cheqnum','$bank[accnum]', '".USER_DIV."')";
					...
					$save_sql2 = "
								INSERT INTO supp_payment_print_items 
									(payment_id, supid, purchase, tdate, sdate, paid_amt, sett_amt) 
								VALUES 
									('$supp_pay_id','$supid','$invids[$key]','$date','now','$paidamt[$key]','$stock_setamt[$key]')";
	...
	if($setamt > 0){
		...
		$Sl = "
				INSERT INTO sup_stmnt 
					(supid, amount, edate, descript,ref,cacc, div) 
				VALUES 
					('$sup[supid]','".sprint($setamt - ($setamt * 2))."','$sdate', 'Settlement Discount','$cheqnum','$bank[accnum]', '".USER_DIV."')";
		$Rs = db_exec($Sl) or errDie("Unable to insert statement record in Cubit.",SELF);

		#record this paid settlement discount for reporting ...
		$settl_sql = "
					INSERT INTO settlement_sup 
						(supplier,amt,setamt,setvatamt,setvat,setvatcode,tdate,sdate,refnum) 
					VALUES 
						('$sup[supid]','$amt','$setamt','$setvatamt','$setvat','$setvatcode','$accdate','now','$refnum')
					";
