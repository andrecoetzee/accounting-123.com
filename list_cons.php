...
function listContacts() {
	...
	// execute the query
	$sql = "SELECT id, name, surname, title, accountname, tell, email FROM cons
		WHERE $pgref
			AND $sql_filters
			AND ( (
				assigned_to = '$_SESSION[USER_NAME]'
	...
	if ( $contact_data == "" ) {
		$contact_data = "<tr bgcolor='$cellcolor[0]'><td colspan=6>No contacts found</td></tr>";
	}
	...
	<table width='100%' border=0 cellpadding='".TMPL_tblCellPadding."' cellspacing='".TMPL_tblCellSpacing."'>
	<tr>
		<td align=left nowrap><font size=2><b>";

	if ( $action == "viewcon" ) {
		$OUTPUT .= "<a class=nav href=\"new_con.php\">New Contact</a></b></font>";
	}
