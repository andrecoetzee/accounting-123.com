...
//	$fdate = date("Y")."-".date("m")."-"."01";
	...
	# Query server
	$sql = "SELECT * FROM sup_stmnt WHERE supid = '$supid' AND edate >= '$from_date' AND edate <= '$to_date' AND div = '".USER_DIV."' ";
	$stRslt = db_exec ($sql) or errDie ("Unable to retrieve invoices statement from database.");
	if (pg_numrows ($stRslt) < 1) {
		$stmnt[] = array(
			'date' => "No previous invoices/transactions for this month.", 
			'ref' => "  ", 
			'cacc' => "  ", 
			'descript' => "  ", 
			'amount' => "  "
