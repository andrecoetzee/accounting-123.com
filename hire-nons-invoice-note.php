...
// 		}
		...
		$products .= "
					<tr bgcolor='".bgcolorg()."'>
						...
						<td>$stkd[unitcost]</td>
						<td><input type='hidden' name='amts[]' value='$amt[$key]'>".CUR." $amt[$key]</td>
						<td>$acc[topacc]/$acc[accnum] - $acc[accname]</td>
					</tr>";
	}
	$products .= "</table>";
				...
				<tr>
					...
					<td align='right'>
						<table ".TMPL_tblDflts." width='80%'>
							...
							<tr bgcolor='".bgcolorg()."'>
								<td>VAT $vat14</td>
								<td align='right'><input type='hidden' name='vat' value='$VAT'>".CUR." $VAT</td>
							</tr>
							<tr bgcolor='".bgcolorg()."'>
								<th>GRAND TOTAL</th>
								<td align='right'><input type='hidden' name='total' value='$TOTAL'>".CUR." $TOTAL</td>
							</tr>
						</table>
					</td></tr>
					<tr>
...
/*refnum*/
	...
	//	$rslt = db_exec($sql) or errDie("Unable to save the comments to Cubit.");
		...
		$products[$page][] = "
						<tr valign='top'>
							...
							<td align='right'>".CUR." $amts[$key]&nbsp;</td>
						</tr>";
	...
	if($inv['ctyp'] == 's'){
		...
		$sql = "INSERT INTO stmnt(cusnum, invid, amount, date, type, div) VALUES('$inv[cusid]', '$real_noteid', '-$TOTAL','$td', 'Non Stock Credit Note, for invoice $inv[invnum]', '".USER_DIV."')";
		...
		<table cellpadding='0' cellspacing='0' width='85%' style='$table_borders'>
			...
			<table border='0' cellpadding='2' cellspacing='2' width='100%'>
				<tr>
					...
					<td align='right'><font size='5'><b>Credit Note</b></font></td>
				</tr>
			</table>
			</td></tr>
		</table>
