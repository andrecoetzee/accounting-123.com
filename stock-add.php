...
function write($_POST)
{
	...
	$sql = "
		INSERT INTO stock (
			supplier1, supplier2, supplier3, stkcod, stkdes, 
			prdcls, classname, csamt, units, buom, 
			suom, rate, shelf, row, minlvl, 
			maxlvl, csprice, selamt, exvat, catid, 
			catname, whid, blocked, type, serd, 
			alloc, com, bar, div, vatcode, 
			markup, rfidtype, rfidfreq, rfidrate, warranty
		) VALUES (
			'$supplier1', '$supplier2', '$supplier3', '$stkcod', '$stkdes', 
			'$clasid', '$clas[classname]', '0', '0', '$buom', 
			'$suom', '$rate', '$shelf', '$row', '$minlvl', 
			'$maxlvl', '0', '$selamt', 'n', '$catid', 
			'$cat[cat]', '$whid', 'n', '$stktp', '$serd', 
			'0', '0', '$bar', '".USER_DIV."', '$vatcode', 
			'$markup', '$rfidtype', '$rfidfreq', '$rfidrate', '$warranty'
		)";
		...
		while($list = pg_fetch_array($listRslt)){
			...
			$sql = "
				INSERT INTO plist_prices (
					...
					'$list[listid]', '$stkid', '$catid', '$clasid', '$selamt', '".USER_DIV."','Yes'
				)";
		...
		while($list = pg_fetch_array($listRslt)){
			...
			$sql = "
				INSERT INTO splist_prices (
					...
					'$list[listid]', '$stkid', '$catid', '$clasid', '0', '".USER_DIV."'
				)";
			$rslt = db_exec($sql) or errDie("Unable to insert price list items to Cubit.",SELF);
		}
	}
...
//	return $write;
	...
	unset ($_POST);
	$_REQUEST = array ();
	return view ("<li class='yay'>Stock has been added.</li>");
