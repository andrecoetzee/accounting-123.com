...
function details($HTTP_GET_VARS)
{
	...
	if(pg_numrows($stkdRslt) < 1){
		return "<li> There are no undelivered items for this invoice.</li>";
	}
	...
	while($stkd = pg_fetch_array($stkdRslt)){
		...
		$products .= "
			<tr bgcolor='".bgcolorg()."'>
				...
				<td>$stk[stkdes]</td>
				<td>
					<input type='hidden' name='sers[$stkd[stkid]][]' value='$stkd[serno]'>
					$stkd[serno]
				</td>
				<td>
					<input type='hidden' size='4' name='qts[]' value='$stkd[qty]'>
					<input type='text' size='4' name='qtys[]' value='$stkd[qty]'>
				</td>
			</tr>";
	}
	...
	if(strlen($products) < 1){
		return "
			<li class='err'>Delivery Note For All Items Has Already Been Completed.</li><br>"
			.mkQuickLinks(
	...
	while($stkd = pg_fetch_array($stkdRslt)){
		...
		$products .= "
			<tr bgcolor='".bgcolorg()."'>
				...
				<td>
					<input type='hidden' name='ids[]' value='$stkd[id]'>
					<input type='hidden' name='stkids[]' value='$stk[stkid]'>$stk[stkcod]
				</td>
				<td>$stk[stkdes]</td>
				<td><input type='hidden' name='sers[$stkd[stkid]][]' value='$stk[serno]'>$stk[serno]</td>
				<td>
					<input type='hidden' size='4' name='qts[]' value='$stkd[qty]'>
					<input type='text' size='4' name='qtys[]' value='$stkd[qty]'>
				</td>
...
function confirm($HTTP_POST_VARS)
{
	...
	$v->isOk ($del_day, "num", 1, 2, "Invalid Invoice Date day.");
	$v->isOk ($del_month, "num", 1, 2, "Invalid Invoice Date month.");
	$v->isOk ($del_year, "num", 1, 5, "Invalid Invoice Date year.");
	$odate = $del_day."-".$del_month."-".$del_year;
...
/* --- ----------- Clac --------------------- */
	...
	$details = "
		...
		<form action='".SELF."' method='POST'>
			...
			<input type='hidden' name='invid' value='$invid'>
			<input type='hidden' name='del_day' value='$del_day'>
			<input type='hidden' name='del_month' value='$del_month'>
			<input type='hidden' name='del_year' value='$del_year'>
		<table ".TMPL_tblDflts." width='95%'>
			<tr>
				<td valign='top'>
					<table ".TMPL_tblDflts.">
			...
			<tr>
				<td valign='top' width='30%'>
					<table ".TMPL_tblDflts.">
						...
						<tr><td>Customer Order No: $inv[cordno]</td></tr>
					</table>
				</td>
