...
/* --- Start Products Display --- */
	...
	$products = "
		<script>
			function calcval (entry_id){
				var dem1id = '1'+entry_id;
				var dem2id = '2'+entry_id;

				var proc1 = document.getElementById(dem1id).value;
				var proc2 = document.getElementById(dem2id).value;

				var proc1last = proc1.substr(-2);
				var proc2last = proc2.substr(-2);
			
				proc1 = proc1*1;
				proc2 = proc2*1;

//				proc1lastint = proc1last * 1;
//				proc2lastint = proc2last * 1;

//				if (proc1lastint <= 25 && proc1lastint > 0){
//					proc1 = proc1 + (25 - proc1lastint);
//				}else {
//					if (proc1lastint <= 50 && proc1lastint > 0){
//						proc1 = proc1 + (50 - proc1lastint);
//					}else {
//						if (proc1lastint <= 75 && proc1lastint > 0){
//							proc1 = proc1 + (75 - proc1lastint);
//						}else {
//							if (proc1lastint > 0)
//								proc1 = proc1 + (100 - proc1lastint);
//						}
//					}
//				}

//				if (proc2lastint <= 25 && proc2lastint > 0){
//					proc2 = proc2 + (25 - proc2lastint);
//				}else {
//					if (proc2lastint <= 50 && proc2lastint > 0){
//						proc2 = proc2 + (50 - proc2lastint);
//					}else {
//						if (proc2lastint <= 75 && proc2lastint > 0){
//							proc2 = proc2 + (75 - proc2lastint);
//						}else {
//							if (proc2lastint > 0)
//								proc2 = proc2 + (100 - proc2lastint);
//						}
//					}
//				}

				//set the new dimensions
//				document.getElementById(dem1id).value=proc1;
//				document.getElementById(dem2id).value=proc2;

				return ((proc1 * proc2)/1000)/1000;
			}
		</script>
	...
	while($stkd = pg_fetch_array($stkdRslt)){
		...
		if($stkd['account'] != 0) {
			...
			$qtyin = "<input type='text' size='5' name='qtys[$ai]' value='$stkd[qty]'>";
			...
			# Input qty if not serialised
			$qtyin = "<input id='$ai' type='text' size='5' name='qtys[$ai]' value='$stkd[qty]'>";
			if($stk['serd'] == 'yes'){
				$qtyin = "<input id='$ai' type='hidden' size='5' name='qtys[$ai]' value='$stkd[qty]'>$stkd[qty]";
			}
			...
			# this shows stock based entries from the db ....
			$products .= "
				<tr bgcolor='".bgcolorg()."'>
					<td>
					...
					<td nowrap><input id='1$ai' type='text' size='4' name='dem1[$ai]' value='$stkd[dem1]' onFocus=\"document.getElementById('$ai').value=calcval($ai);\"> <input id='2$ai' type='text' size='4' name='dem2[$ai]' value='$stkd[dem2]' onFocus=\"document.getElementById('$ai').value=calcval($ai);\"></td>
					<td>$qtyin</td>
					<td>$viewcost</td>
	...
	if(isset($whidss)){
		foreach($whidss as $key => $whid){
			if(isset($stkidss[$key]) && $stkidss[$key] != "-S" && isset($cust['pricelist'])) {
				...
				# this shows new stock entries
				$products .= "
					<tr bgcolor='".bgcolorg()."'>
						<td>
							<input type='hidden' name='accounts[$ai]' value='0'>
							<input type='hidden' name='descriptions[$ai]' value=''>
							<input type='hidden' name='whids[]' value='$whid'>$wh[whname]
						</td>
						...
						<td>$sernos</td>
						<td>".extlib_rstr($stk['stkdes'], 30)."</td>
						<td nowrap><input id='1$ai' type='text' size='4' name='dem1[$ai]' value='$dem1[$key]' onFocus=\"document.form.qtemp.value=calcval($ai);\"> <input id='2$ai' type='text' size='4' name='dem2[$ai]' value='$dem2[$key]' onFocus=\"document.form.qtemp.value=calcval($ai);\"></td>
						<td>$qtyin</td>
						<td>$viewcost</td>
				...
				# Input qty if not serialised
				$qtyin = "<input type='text' size='5' name='qtemp' value='$qtyss[$key]'>";
...
//								}
						...
						$products .= "
							<tr bgcolor='".bgcolorg()."'>
								<td>
									...
									<input type='hidden' name='whidss[$ai]' value='$whid'>$wh[whname]
								</td>
								<td>$cods</td>
						...
						$products .= "
							<tr bgcolor='".bgcolorg()."'>
								...
								<td></td>
								<td><input type='text' size='5' name='qtyss[$ai]' value='1'></td>
...
/*--- Start checks --- */
	...
	if (empty($ajax_err) && (isset($cusnum) || AJAX)) {
		if (isset($cusnum)) {
			...
			$custedit = "
				<td nowrap>
					<a href='javascript: popupSized(\"customers-new.php?cusnum=$cusnum&onthespot=$OTS_OPT\", \"edit_cust\", 700, 630);'>
...
//				</tr>
	...
	$details_end = "
				...
				<tr>
					<td>
						...
						<table ".TMPL_tblDflts." width='100%'>
							...
							<tr>
								<td bgcolor='".bgcolorg()."'><a href='customers-new.php?re=$inv[invid]'>New Customer</a></td>
								<td bgcolor='".bgcolorg()."' rowspan='5' align='center' valign='top'><textarea name='comm' rows='4' cols='20'>$comm</textarea></td>
							</tr>
							<tr>
				...
				<tr>
					<td align='right'><input name='diffwhBtn' type='submit' value='Different Store'> | <input name='addprodBtn' type='submit' value='Add Product'> | <input name='addnon' type='submit' value='Add Non stock Product'> | <input type='submit' name='saveBtn' value='Save'> </td>
...
function write($_POST)
{
	...
	if(isset($qtys)){
		foreach($qtys as $keys => $qty){
			...
			$qty = sprint3 ($qty);
//			if (strlen (substr($qty,strpos($qty,"."))) > 4)
//				$qty = 
//			print substr($qty,strpos($qty,"."))."<br>";
			...
			if($qty <= 0){
				$v->isOk ($qty, "float", 0, 0, "Error : Item Quantity must be more than zero. Product number : <b>".($keys+1)."</b>");
			}
		}
	}
	# check whids
			...
			if(isset($remprod)&&in_array($keys, $remprod)){
				...
				$sql = "
					INSERT INTO inv_items (
						...
						vatcode, del, dem1, dem2
					) VALUES (
						'$invid', '$whids[$keys]', '$stkids[$keys]', '$qtys[$keys]', '$unitcost[$keys]', '$amt[$keys]', 
						'$disc[$keys]', '$discp[$keys]', '$sernos_ss[$keys]', '$sernos[$keys]', '".USER_DIV."', 
						'$vatcodes[$keys]', '0', '$dem1[$keys]', '$dem2[$keys]'
					)";
				$rslt = db_exec($sql) or errDie("Unable to insert invoice items to Cubit.",SELF);
...
// 		}
	...
	if($chrgvat == "exc"){
		...
		if ($traddisc_setting == "include") $tradvar = $subtotal;
		else $tradvar = $sub;
...
//move('cust-credit-stockinv-newsetting.php');
			$OUTPUT = "
				<script>
					...
					move('settings/cust-credit-stockinv-newsetting.php?vol=yes');
				</script>";
		} else {
//		move('cust-credit-stockinv.php');
//move('cust-credit-stockinv-newsetting.php');
