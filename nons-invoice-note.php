...
/*refnum*/
	...
	//	$VATP = TAX_VAT;
		...
		$sql = "
			INSERT INTO nons_note_items (
				noteid, qty, description, amt, 
				unitcost, vatcode
			) VALUES (
				'$noteid', '$qtys[$key]', '$stkd[description]', '$amts[$key]', 
				'$stkd[unitcost]', '$stkd[vatex]'
			)";
		$stkdRslt = db_exec($sql);
	...
	if($inv['ctyp'] == 's'){
		...
		$sql = "
			INSERT INTO stmnt (
				...
				'$inv[cusid]', '$real_noteid', '-$TOTAL','$td', 
				'Non Stock Credit Note, for invoice $inv[invnum]', '".USER_DIV."', '$inv[odate]'
			)";
		$stmntRslt = db_exec($sql) or errDie("Unable to insert statement record in Cubit.",SELF);

		# Update the customer (make balance less)
		$sql = "UPDATE customers SET balance = (balance - '$TOTAL'::numeric(16,2)) WHERE cusnum = '$inv[cusid]' AND div = '".USER_DIV."'";
		$rslt = db_exec($sql) or errDie("Unable to update invoice in Cubit.",SELF);

		# Update the customer (make balance less)
		$sql = "UPDATE open_stmnt SET balance = (balance - '$TOTAL'::numeric(16,2)) WHERE invid = '$inv[invnum]' AND div = '".USER_DIV."'";
		$rslt = db_exec($sql) or errDie("Unable to update invoice in Cubit.",SELF);
		...
		$sql = "
			INSERT INTO cashbook (
				...
				descript, cheqnum, amount, banked, accids, 
				amounts, chrgvats, vats, div,accinv
			) VALUES (
				'$inv[jobid]', 'withdrawal', '$td', '$inv[cusname]', 
				'Nons Stock Credit note for invoice $inv[invnum]', '0', '$TOTAL', 'no', '', 
				'0', '$inv[chrgvat]', '0', '".USER_DIV."','$stkacc'
			)";
		die($sql);
	...
	db_connect();
	$sql = "UPDATE nons_invoices SET balance = (balance - '$TOTAL'::numeric(16,2)) WHERE invid = '$invid' AND div = '".USER_DIV."'";
	$upRslt = db_exec($sql) or errDie ("Unable to update invoice information");

	# write note
	$sql = "
		INSERT INTO nons_inv_notes (
			invid, invnum, cusname, cusaddr, cusvatno, 
			chrgvat, date, subtot, vat, total, 
			username, prd, notenum, ctyp, remarks, 
			div, bankid
		) VALUES (
			'$inv[invid]', '$inv[invnum]', '$inv[cusname]', '$inv[cusaddr]', '$inv[cusvatno]', 
			'$inv[chrgvat]', '$td', $SUBTOT, $VAT, $TOTAL, 
			'".USER_NAME."', '".PRD_DB."', '$real_noteid', '$inv[ctyp]', '$remarks', 
			'".USER_DIV."', '$inv[bankid]'
		)";
	...
	$Sl = "
		INSERT INTO sj (
			cid, name, des, 
			date, exl, vat, inc, div
		) VALUES (
			'$inv[cusid]', '$inv[cusname]', 'Credit Note: $real_noteid, Invoice $inv[invnum]', 
			'$td', '".-sprint($TOTAL-$VAT)."', '-$VAT', '".-sprint($TOTAL)."', '".USER_DIV."'
		)";
	$Ri = db_exec($Sl);
...
function custfCT($amount, $cusnum,$age)
{
		...
		if($amount < 0){
			...
			$sql = "
				INSERT INTO custran (
					...
					'$cusnum', '$odate', '$amount', '".USER_DIV."','$age'
				)";
	...
	$sql = "DELETE FROM custran WHERE balance = 0::numeric(16,2) AND fbalance = 0::numeric(16,2) AND div = '".USER_DIV."'";
	$rs = db_exec($sql);
}
