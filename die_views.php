  ...
  if ($mon==12){$td=31;$M='December';}
   $Month=$M;
      ...
      if ($day=="00") {
       ...
       if ($For =='All') {$Whe="WHERE substr(date,3,2)='$mon' and substr(date,5,4)='$year'";} else {$Whe="WHERE ref='$For'and substr(date,3,2)='$mon' and substr(date,5,4)='$year'";}
        db_conn('cubit');
        $Sql = "SELECT date,start,last,des,by,made,ref FROM die $Whe ORDER BY substr(date,1,2) ASC,start";
	$Rslt = db_exec ($Sql) or errDie ("Unable to access database");
	$numrows = pg_numrows($Rslt);
        if ($For =='All') {$V="<th>Person</th>";} else {$V="";}
	if ($numrows < 1) {
        ...
        $Tab = "
		<table border=0 cellpadding='".TMPL_tblCellPadding."' cellspacing='".TMPL_tblCellSpacing."'>
		<tr>$V<th>Date</th><th>Time</th><th>Description</th><th>Made By</th><th>Date Made</th></tr>
		";
			...
			if ($i % 2) {                                                              // every other row gets a diff color
				...
				$bgColor = TMPL_tblDataColor2;
                      ...
                      if ($For =='All') {$V="<td>$per</td>";} else {$V="";}
                        ...
                        $Tab .= "<tr bgcolor='$bgColor'>$V<td>$day</td><td>$time</td><td>$Data[des]</td><td>$Data[by]</td><td>$Day $M $Year</td></tr>";
		};}
		$Tab .= "</table>";
...
";
       ...
       if ($For =='All') {$Whe="WHERE substr(date,1,2)='$day' and substr(date,3,2)='$mon' and substr(date,5,4)='$year'";} else {$Whe="WHERE substr(date,1,2)='$day' and ref='$For'and substr(date,3,2)='$mon' and substr(date,5,4)='$year'";}
        db_conn('cubit');
        $Sql = "SELECT date,start,last,des,by,made,ref FROM die $Whe ORDER BY substr(date,1,2) ASC,start";
	$Rslt = db_exec ($Sql) or errDie ("Unable to access database");
	$numrows = pg_numrows($Rslt);
	...
	if ($numrows < 1) {
        ...
        $Tab = "
		<table border=0 cellpadding='".TMPL_tblCellPadding."' cellspacing='".TMPL_tblCellSpacing."'>
		<tr>$V<th>Time</th><th>Description</th><th>Made By</th><th>Date Made</th></tr>
		";
			...
			if ($i % 2) {                                                              // every other row gets a diff color
				...
				$bgColor = TMPL_tblDataColor2;
                      ...
                      if ($For =='All') {$V="<td>$per</td>";} else {$V="";}
                        ...
                        $Tab .= "<tr bgcolor='$bgColor'>$V<td>$time</td><td>$Data[des]</td><td>$Data[by]</td><td>$Day $M $Year</td></tr>";
		};}
		$Tab .= "</table>";
