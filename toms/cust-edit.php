	...
	switch ($HTTP_POST_VARS["key"]) {
			...
			default:
				if (isset($HTTP_GET_VARS['custid'])){
					$OUTPUT = edit ($HTTP_GET_VARS['custid']);
				} else {
...
function confirm ($HTTP_POST_VARS)
{
	...
	$v->isOk ($chrgvat, "string", 1, 3, "Invalid charge vat.");
...
function write ($HTTP_POST_VARS)
{
	...
	# write to db
	$sql = "UPDATE customers SET accno = '$accno', surname = '$surname', title = '$title', init = '$init',";
	$sql .= "firstname = '$firstname', category = '$catid', class = '$clasid', paddr = '$paddr', daddr = '$daddr',";
	$sql .= "contname = '$contname', bustel = '$bustel', hometel = '$hometel', cellno = '$cellno', faxno = '$faxno',";
	$sql .= "email = '$email', saleterm = '$saleterm', traddisc = '$traddisc', setdisc = '$setdisc', pricelist = '$listid',";
	$sql .= "chrgint = '$chrgint', overdue = '$overdue', chrgvat = '$chrgvat', vatinc = '$vatinc', credterm = '$credterm',";
	$sql .= "odate = '$odate', credlimit = '$credlimit', blocked = '$block' WHERE custid  = '$custid'";

	$custRslt = db_exec ($sql) or errDie ("Unable to edit customer on the system.", SELF);
	if (pg_cmdtuples ($custRslt) < 1) {
		return "<li class=err>Unable to Edit customer in database.";
	}
	$write =
	"<table border=0 cellpadding='".TMPL_tblCellPadding."' cellspacing='".TMPL_tblCellSpacing."' width='50%'>
	<tr><th>Customer edited</th></tr>
	<tr class=datacell><td>Customer <b>$firstname $surname</b>, has been edited.</td></tr>
	</table>
	<p>
	<table border=0 cellpadding='2' cellspacing='1'>
	<tr><th>Quick Links</th></tr>
