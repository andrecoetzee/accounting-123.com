...
function get_project ()
{
	...
	if(pg_numrows($run_pro) < 1){
		...
		while ($parr = pg_fetch_array ($run_pro)){
			$pro_drop .= "<option value='$parr[id]'>$parr[project_name]</option>";
		}
		$pro_drop .= "</select>";
	}

	$display = "
					<h2>Select Project To View/Edit</h2>
					<table ".TMPL_tblDflts.">
...
function enter ($HTTP_POST_VARS,$err="")
{
	...
	if(!isset($project) OR (strlen($project) < 1)){
		return "<li class='err'>Invalid Project Selected.</li>";
	}
	...
	$get_lev2 = "SELECT * FROM sub_projects WHERE project_id = '$project' ORDER BY sub_project_name";
	...
	$get_pro1 = "SELECT * FROM projects WHERE id = '$project' ORDER BY project_name";
