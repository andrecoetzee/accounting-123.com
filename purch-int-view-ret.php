...
function slct()
{
	...
	//layout
	$slct = "
				<h3>View Returned International Stock Orders</h3>
...
#	rather make a simple array of all months ?
...
#	for ($i = $from_month; $i <= $to_month; $i++) {
	...
	foreach ($date_arr as $i){
		...
		$queries[] = "SELECT *,'$schema' AS query_schema FROM \"$schema\".purchint_ret WHERE rdate >= '$fromdate' AND rdate <= '$todate' AND div = '".USER_DIV."'";
	}
	...
	//	why overwrite a perfectly good prd setting ???
	//	$prd = (int)$date[1];

		db_conn($prd);
		$sql = "SELECT * FROM purch_int WHERE purid='$stkp[purid]'";
		...
		$printOrd .= "
						<tr class='".bg_class()."'>
							...
							<td>$date</td>
							<td>$stkp[supname]</td>
							<td align='right'>$symbol $stkp[subtot]</td>
							<td><a href='purch-int-det-ret.php?rpurid=$stkp[rpurid]&prd=$prd'>Details</a></td>
						</tr>";
		$i++;
	}
	$tot = sprint($tot);
