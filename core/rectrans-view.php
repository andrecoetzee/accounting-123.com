...
function viewtrans($HTTP_POST_VARS)
{
	...
	if($search == "date"){
		...
		$search = "SELECT * FROM rectrans WHERE date >= '$fromdate' AND date <= '$todate' AND div = '".USER_DIV."' ORDER BY refnum ASC";
	}
	...
	if($search == "refnum"){
		...
		# create the Search SQL
		$search = "SELECT * FROM rectrans WHERE refnum >= $fromnum AND refnum <= $tonum AND div = '".USER_DIV."' ORDER BY refnum ASC";
	}

	# View All
	if($search == "all"){
		# create the Search SQL
		$search = "SELECT * FROM rectrans WHERE div = '".USER_DIV."' ORDER BY refnum ASC";
	}
	...
	$OUTPUT = "
		...
		<form action='rectrans-run.php' method='POST'>
		<table ".TMPL_tblDflts.">
			<tr>
				<th>Date</th>
				<th>Debit</th>
	...
	if (pg_numrows ($tranRslt) < 1) {
		return "
			<li> No Recurring Transactions Found.</li>
			<p>
			<table ".TMPL_tblDflts." width='25%'>
				<tr>
					<th>Quick Links</th>
	...
	while ($tran = pg_fetch_array ($tranRslt)){
		...
		$OUTPUT .= "
			<tr bgcolor='".bgcolorg()."'>
				...
				<td>$details</td>
				<td>$chrgvat</td>
				<td><input type='checkbox' name='run[]' value='$recid'></td>
				<td><a href='rectrans-edit.php?recid=$recid'>Edit</a></td>
			</tr>";
	}
	...
	$OUTPUT .= "
		<tr>
			<td colspan='6'><br></td>
			<td align='right'><input type='submit' value='Process Selected' name='proc'></td>
			<td align='right'><input type='submit' value='Remove Selected' name='rem'></td>
		</tr>
		</form>
		</table>";
	$OUTPUT .= "
