...
define ("TMPL_fntFamily", "sans-serif");
...
$conn = pg_connect ("user=".DB_USER." password=".DB_PASS." dbname=".$database) or errDie ("Unable to connect to database server.", SELF);
$rslt = db_exec($conn, $sql);
        return $rslt;
}
...
if(pg_numrows($rslt) < 1){
        errDie("The Account link for $filter was not found, Please Set all account links on the Current module",SELF);
}
