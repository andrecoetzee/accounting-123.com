...
function display()
{
	...
	for ($i = 1; $i <= 14; $i++) {
		$union[] = "
		SELECT supname, id, descript, edate, credit, debit, cbalance,
			dbalance, topacc, accnum, accname
		FROM \"$i\".suppledger
			LEFT JOIN cubit.suppliers
				ON suppledger.supid=suppliers.supid
			LEFT JOIN core.accounts
				ON suppledger.contra=accounts.accid
	...
	$OUTPUT = "
	<center>
	<h3>Creditors - Transaction Detail Report</h3>
