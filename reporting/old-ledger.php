...
function viewtran($_POST)
{
	...
	foreach($accids as $key => $accid){
		...
		$trans .= "<tr bgcolor='".TMPL_tblDataColor2."'><td colspan=8><b>$acc[topacc]/$acc[accnum] - $acc[accname]</b></td></tr>";
		$trans .= "<tr bgcolor='".TMPL_tblDataColor2."'><td colspan=2><br></td><td>Br/Forwd</td><td>Brought Forward</td><td align=right>$bal[dbalance]</td><td align=right>$bal[cbalance]</td><td align=right>$balance $fl</td><td> </td></tr>";
		...
		while($tran = pg_fetch_array($tranRs)){
			...
			$trans .= "<tr bgcolor='".TMPL_tblDataColor1."'><td><br></td><td>$tran[edate]</td><td>$tran[eref]</td><td>$tran[descript]</td><td align=right>$tran[debit]</td><td align=right>$tran[credit]</td><td align=right>$cbalance $cfl</td><td>$tran[ctopacc]/$tran[caccnum] - $tran[caccname]</td></tr>";
		}
