...
function sub()
{
	...
	<input type=submit name=key value='Add More Sub Headings'> <input type=submit name=key value='Submit'>
	</form>
...
function printacc($_POST)
{
	...
	# Connect to database
	core_connect();
	$sql = "SELECT DISTINCT topacc,accnum FROM trial_bal WHERE topacc >= ".MIN_BAL."";
	...
	<tr><td> </td><td align=center><input type=submit value='Continue &raquo'></td></tr>
	</table>
	</form>
	<p>
...
function write($_POST)
{
	...
	core_connect();
	$sql = "DELETE FROM bal_sheet WHERE div = 11111111";
	...
	foreach($oesub as $oref => $sub){
		$query = "INSERT INTO bal_sheet(type, ref, value, div) VALUES('OESUB', '$oref', '$sub', 11111111)";
		...
		foreach($subaccs as $kkey => $subacc){
			$query = "INSERT INTO bal_sheet(type, ref, value, div) VALUES('OEACC','$oref','$subacc', 11111111)";
	...
	if(isset($asssub)) {
		foreach($asssub as $aref => $sub){
			$query = "INSERT INTO bal_sheet(type, ref, value, div) VALUES('ASSSUB', '$aref', '$sub', 11111111)";
			$bsRslt = db_exec($query) or errDie("Unable to insert Balance Sheet settings to database",SELF);
			$subaccs = getaccs($aref, $heads, 'a', $accno);
			foreach($subaccs as $kkey => $subacc){
				$query = "INSERT INTO bal_sheet(type, ref, value, div) VALUES('ASSACC','$aref','$subacc', 11111111)";
				$accRslt = db_exec($query) or errDie("Unable to insert Balance sheet settings to Cubit.",SELF);
			}
		}
	}
