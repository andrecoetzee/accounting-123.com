	...
	switch ($HTTP_POST_VARS["key"]) {
		...
		default:
			if (isset($HTTP_GET_VARS['cusnum'])){
				$OUTPUT = bblock ($HTTP_GET_VARS['cusnum']);
			} else {
...
function bblock($cusnum)
{
	...
	$sql = "SELECT * FROM customers WHERE cusnum = '$cusnum' AND ddiv = '".USER_DIV."'";
	...
	$rem = "
		...
		<table cellpadding='0' cellspacing='0'>
			<tr>
				<th colspan='2'>Confirm Unblock Customer : Customer Details</th>
			</tr>
			<tr valign='top'>
				<td>
					<table ".TMPL_tblDflts.">
...
function write ($HTTP_POST_VARS)
{
	...
	# Write to db
	$sql = "UPDATE customers SET blocked = 'no', div = ddiv, ddiv = 0 WHERE cusnum = '$cusnum' AND ddiv = '".USER_DIV."'";
	$custRslt = db_exec ($sql) or errDie ("Unable to Unblock Customer.", SELF);
	if (pg_cmdtuples ($custRslt) < 1) {
		return "<li class='err'>Unable to Unblock Customer.</li>";
	}

	$write = "
		<table ".TMPL_tblDflts." width='50%'>
			<tr>
				<th>Customer Unblocked</th>
			</tr>
			<tr class='datacell'>
				<td>Customer <b>$cusname $surname</b>, has been Unblocked.</td>
			</tr>
		</table>
		<p>
		<table border='0' cellpadding='2' cellspacing='1'>
