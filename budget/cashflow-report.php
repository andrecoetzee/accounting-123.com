...
function details($_GET)
{
	...
	$invtabs = array("invoices", "nons_invoices","cf");
	$purtabs = array("purchases", "purch_int", "nons_purch_int", "nons_purchases","cf");
...
function getinvdays($tabs, $db, $id, $date, $days){
	...
	foreach($tabs as $key => $tab){
		if($tab!="cf") {
			...
			if($fdays==-1) {
				$sq = "SELECT id,amount FROM cf WHERE (date- date '$tdate') < 1 AND amount>0";
			} else {
				$sq = "SELECT id,amount FROM cf WHERE (date- date '$tdate') > $fdays AND (date- date '$tdate') <= $days AND amount>0";
			}
			$rs = db_exec($sq);
			if(pg_numrows($rs) > 0){
				while($td = pg_fetch_array($rs)){
					//print "working";
					$ret[] = array("tab" => $tab, $id => $td['id'], "amt" => $td['amount']);
				}
			}
		}
	}
