	...
	function fetch_html() {
		...
		foreach ( $this->nodes as $arr => $arrval ) {
			if ( $arrval != "_end" ) {
				...
				switch ( $type ) {
					case "A": // account
						$name = "<a class='mailtree' href='javascript:treeAjaxLink(\"iframe.php\", \"script=accounts.php&key=edit&aid=$id\");'>$name</a>";
						break;
					case "F": // folder
						$name = "<a class='mailtree' href='javascript:treeAjaxLink(\"iframe.php\", \"script=messages.php&key=frameset&fid=$id&print=1\");'>$name</a>";
						break;
				}
