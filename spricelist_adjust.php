...
// Quick Links
$OUTPUT .= mkQuickLinks (
	ql("toms/sup-pricelist-add.php", "Add Supplier Pricelist"),
	ql("toms/sup-pricelist-view.php", "View Supplier Pricelists")
);
...
function enter($errors="")
{
	...
	// Pricelist dropdown ------------------------------------------------
	$sql = "SELECT listid, listname FROM exten.spricelist ORDER BY listname ASC";
	...
	while ($classification_data = pg_fetch_array($classification_rslt)) {
		...
		$classification_sel .= "
		<option value='$classification_data[clasid]' $sel>
			$classification_data[classname]
		</option>";
	}
...
function confirm()
{
	...
	if ($pricelist) {
		$sql = "SELECT listname FROM exten.spricelist WHERE listid='$pricelist'";
