...
function slip ($HTTP_POST_VARS, $pure = false)
{
	...
	if (pg_numrows ($pRslt) > 0) {
		...
		$run_deds = db_exec ($sql) or errDie ("Unable to check employee information.");

		$check_emp_list = array ();
		while ($checkarr = pg_fetch_array ($run_deds)){
			$check_emp_list[] = "payslip = '$checkarr[id]'";
		}

		$search_slips = implode (" OR ", $check_emp_list);

		$colspan=0;
		$heading_arr = array();

		$get_check_deds = "SELECT distinct(type) FROM emp_ded WHERE $search_slips";
		$run_check_deds = db_exec ($get_check_deds) or errDie ("Unable to get employee deduction information.");
		if (pg_numrows ($run_check_deds) > 0){
			while ($check_ded_header_arr = pg_fetch_array ($run_check_deds)){
				$check_ded_header_arr['type'] += 0;
				if ($check_ded_header_arr['type'] > 0) {
					$get_name = "SELECT deduction FROM salded WHERE id = '$check_ded_header_arr[type]' LIMIT 1";
					$run_name = db_exec ($get_name) or errDie ("Unable to get description information.");
					$dedname = pg_fetch_result ($run_name,0,0);
					$deduction_heading .= "<th>$dedname</th>";
					$heading_arr[] = $check_ded_header_arr['type'];
					$colspan++;
				}
			}
		}
...
// 				$ed["totded"] -= $pay["totded"];
			...
			if ($pay['paytype'] == "salp"){
				$ed["totded"] = array();
				foreach ($heading_arr AS $each => $own){
					$get_amt = "
						SELECT amount, description FROM emp_ded 
						WHERE payslip = '$pay[id]' AND type = '$own' 
						LIMIT 1";
					...
					$totals[$own] += pg_fetch_result ($run_amt,0,0);
				}
			}
...
// 		$totded = sprint($totded);
		...
		$displaytotals = "";
		foreach ($totals AS $each => $own){
			$displaytotals .= "<td nowrap><b>".CUR." ".sprint($own)."</b></td>";
		}
