...
function view ()
{
	...
	$view = "
        ...
        <tr  bgcolor='".TMPL_tblDataColor2."'><td>Select Customer Initial</td>
        <td>
        <select name=alpha>
...
function select($HTTP_POST_VARS)
{
	# get vars
	...
	if ($numrows < 1) {
		return "<li class=err>No customers names starting with <b>$ALPHA</b> in database.";
	}
	...
	for ($i=0; $i < $numrows; $i++) {
		...
		$select .= "<tr bgcolor='$bgColor'><td align=center>$myCust[cusnum]</td><td align=center><a href='cust-credit-invoice.php?cusnum=$myCust[cusnum]'>$myCust[cusname]</a></td></tr>";
	}
	$select .= "</table>
