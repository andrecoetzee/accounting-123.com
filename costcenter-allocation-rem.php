...
function get_allocation ($_POST,$err="")
{
	...
	#get template links
	$get_costs = "SELECT ccid FROM costcenters_links WHERE project1 = '$project' AND project3 = '$subsub'";
	$run_costs = db_exec($get_costs) or errDie("Unable to get cost center information.");
	if(pg_numrows($run_costs) > 0){
		while ($clarr = pg_fetch_array($run_costs)){
			$get_cc = "SELECT centername,centercode FROM costcenters WHERE ccid = '$clarr[ccid]'";
			...
			if(pg_numrows($run_cc) < 1){
				...
				while ($carr = pg_fetch_array ($run_cc)){
					...
					$exist_sql = "SELECT * FROM costcenters_links WHERE project1 = '$project' AND ccid = '$clarr[ccid]' AND project3 = '$subsub'";
					$run_exist = db_exec($exist_sql) or errDie("Unable to check or cost center link.");
					if(pg_numrows($run_exist) > 0){
						$get_cc2 = "SELECT centercode,centername FROM costcenters WHERE ccid = '$clarr[ccid]' LIMIT 1";
						$run_cc2 = db_exec($get_cc2) or errDie("Unable to get available cost centers");
						if(pg_numrows($run_cc2) < 1){
							$ccode = "";
							$cname = "";
						}else {
							$ccarr = pg_fetch_array($run_cc2);
						...
						$listing .= "
										<tr class='".bg_class()."'>
											<td>$ccode</td>
											<td>$cname</td>
											<td><input type='checkbox' name='adds[]' value='$clarr[ccid]'></td>
										</tr>
									";
					}
				}
...
//	}
	...
	$display = "
					...
					<form action='".SELF."' method='POST'>
						...
						".TBL_BR."
						<tr>
							<td colspan='3' align='right'><input type='submit' value='Remove Cost Center(s) From Project'></td>
						</tr>
					</form>
