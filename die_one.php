...
function get_die ()
{
	...
	while ($namesA = pg_fetch_array ($ServRslt)) {
        if ($namesA['username']==$user) {$sel="selected";} else{$sel="";}
	$users .= "<option $sel value='$namesA[username]'>$namesA[username]</option>\n";}
	$users .= "</select>\n";
...
function con_die ($_POST)
{
  ...
  $start=$shour.$smin;
     ...
     $date=$day.$mon.$year; //(('$start'<=last) and ('$last'>=start))and

     $user=USER_NAME;

     db_conn('cubit');
     $Sql = "SELECT id FROM die WHERE ((('$start'<last) and ('$last'>start)) and ref='$For' and date='$date')";
	...
	<tr bgcolor='".TMPL_tblDataColor1."'><td><a href='".SELF."'>Make another appointment</a></td></tr>
       ...
       if (($user==$For) and ($numrows>0)) {$c="<h3>$For already has an apointment for the selected time.<p><p><p></h3>";} else {$c="";}
