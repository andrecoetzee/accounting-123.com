...
function view()
{
	...
	<table border=0 cellpadding='".TMPL_tblCellPadding."' cellspacing='".TMPL_tblCellSpacing."' width=400>
		...
		<tr class='bg-odd'><td>Customer type</td><td valign=center><input type=radio size=7 name=ctyp value='loc' checked=yes> Local | <input type=radio size=7 name=ctyp value='int'> International</td></tr>
		<tr><td><br></td></tr>
...
function details($_POST, $error="")
{
	...
	if ($v->isError ()) {
		...
		return $error."</li>".view_err($_POST);
	}
...
/* -- Final Layout -- */
	...
	<tr><td align=right><input name=diffwhBtn type=submit value='Different Store'> | <input name=addprodBtn type=submit value='Add Product'> | <input type=submit name='saveBtn' value='Save'> </td><td>| <input type=submit name='upBtn' value='Update'>$done</td></tr>
	</table><a name=bottom>
	</form></center>";
...
/* --- ----------- Clac --------------------- */
	...
	if (strlen($bar)>0)
	{
		...
		if($Dets['opt']=="No")
		{
			...
			$stid = barext_dbget('stock','bar',$bar,'stkid');
