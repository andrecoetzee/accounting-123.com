...
function get_amount ()
{
	...
	$get_ent = "SELECT id, allocation_linked, allocation_amounts FROM stmnt WHERE id = '$allocate' LIMIT 1";
...
function write_amount ()
{
	...
	$get_ent = "SELECT id, amount, allocation_linked, allocation_amounts FROM stmnt WHERE id = '$allocate' LIMIT 1";
	$run_ent = db_exec ($get_ent) or errDie ("Unable to get entry allocation amount information.");
	if (pg_numrows ($run_ent) < 1){
		return "Invalid Use Of Module.";
	}
...
// 		}
	...
	$get_ent = "SELECT id, allocation_linked, allocation_amounts, amount FROM stmnt WHERE id = '$from' LIMIT 1";
	$run_ent = db_exec ($get_ent) or errDie ("Unable to get entry allocation amount information.");
	if (pg_numrows ($run_ent) < 1){
		return "Invalid Use Of Module.";
	}
	...
	if ($total + $newamount > abs($farr['amount'])){
		return "
			<script>
				window.close();
				window.opener.document.location='debtors-reconciliation-tool.php?from_day=$from_day&from_month=$from_month&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year&customer=$customer&err=<li class=err>Amount too large.</li><br>';
			</script>";
	}
...
*/
	...
	return "
		<script>
			window.close();
			window.opener.document.location='debtors-reconciliation-tool.php?from_day=$from_day&from_month=$from_month&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year&customer=$customer';
		</script>";
