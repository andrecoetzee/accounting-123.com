...
function slct()
{
	...
	if(pg_numrows($whRslt) < 1){
		return "There are no warehouses in Cubit.";
	}else{
...
// AND stkcod='SPG6500E2' ????? wth
	...
	if (pg_numrows ($stkRslt) < 1) {
		return "
			<li class='err'>No stock items found matching criteria.</li>
			<p>
	...
	if (isset($usedate) AND $usedate == "system"){
		$udate = "sdate";
	}else {
		$udate = "edate";
	}
	...
	while ($stk = pg_fetch_array ($stkRslt)){
		...
		$sql = "SELECT sum(qty) as qty, sum(csprice) as csprice, sum(csamt) as csamt FROM stockrec WHERE $udate >= '$fromdate' AND $udate <= '$todate' AND stkid = '$stk[stkid]' AND trantype = 'invoice' AND div = '".USER_DIV."'";
		$recRslt = db_exec($sql) or errDie("Unable to access databse.", SELF);
		$rec = pg_fetch_array($recRslt);

		$sql = "SELECT sum(qty) as qty, sum(csprice) as csprice, sum(csamt) as csamt FROM stockrec WHERE $udate >= '$fromdate' AND $udate <= '$todate' AND stkid = '$stk[stkid]' AND trantype = 'dec' AND div = '".USER_DIV."'";
		$recRslt = db_exec($sql) or errDie("Unable to access databse.", SELF);
		$dec = pg_fetch_array($recRslt);

		$sql = "SELECT sum(qty) as qty, sum(csprice) as csprice, sum(csamt) as csamt FROM stockrec WHERE $udate >= '$fromdate' AND $udate <= '$todate' AND stkid = '$stk[stkid]' AND trantype = 'note' AND div = '".USER_DIV."'";
		$recRslt = db_exec($sql) or errDie("Unable to access databse.", SELF);
		$nrec = pg_fetch_array($recRslt);

		$sql = "SELECT sum(qty) as qty, sum(csprice) as csprice, sum(csamt) as csamt FROM stockrec WHERE $udate >= '$fromdate' AND $udate <= '$todate' AND stkid = '$stk[stkid]' AND trantype = 'purchase' AND div = '".USER_DIV."'";
		$precRslt = db_exec($sql) or errDie("Unable to access databse.", SELF);
		$prec = pg_fetch_array($precRslt);

		$sql = "SELECT sum(qty) as qty, sum(csprice) as csprice, sum(csamt) as csamt FROM stockrec WHERE $udate >= '$fromdate' AND $udate <= '$todate' AND stkid = '$stk[stkid]' AND trantype = 'inc' AND div = '".USER_DIV."'";
		$precRslt = db_exec($sql) or errDie("Unable to access databse.", SELF);
	...
	foreach($items as $key => $item){
		$report .= "
			<tr bgcolor='".bgcolorg()."'>
				...
				<td align='right'>".CUR." $item[profit]</td></tr>";
	}
	...
	if(isset($r_type)){
		$send_rtype = "<input type='hidden' name='r_type' value='$r_type'>";
		$report .= "
			<tr>
				<th colspan='5'>$moveheader</th>
			</tr>
			<tr>
				<th>Stock</th>
				<th>Increase</th>
	...
	$report.= "
	    ...
	    <p>
		<form action='xls/stock-move-report-xls.php' method='POST' name='form'>
			<input type='hidden' name='key' value='view'>
