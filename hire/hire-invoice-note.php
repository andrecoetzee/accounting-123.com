...
//	db_conn('hire');
//
//	$sql = "SELECT * FROM hire_stock_items_reprint  WHERE invnum = '$invid'";
//	$stkdRslt = db_exec($sql);
//	$i = 0;
//
//	while($stkd = pg_fetch_array($stkdRslt)){
...
function write()
{
	...
	$sql = "
		INSERT INTO cubit.stmnt (
			cusnum, invid, amount, date, type, div
		) VALUES (
			'$inv[cusid]', '$noteid', '-$TOTAL', '$sndate', 'Non-Stock Credit Note, for invoice $inv[invnum]', '".USER_DIV."'
		)";
	...
	custledger($inv["cusid"], $hireacc, $sndate, $noteid,
		"Non-Stock Credit Note $noteid", $TOTAL, "c");
	...
	$cc = "
		<script>
			CostCenter('ct', 'Credit Note', '$sndate','Non Stock Credit Note No.$noteid', '".($TOTAL-$inv["vat"])."', '');
		</script>";

	// Reverse the amounts on the coastal reports -----------------------------
	$sql = "
		UPDATE hire.assets_hired SET value=0 WHERE invid='$inv[hire_invid]'";
	...
	vatr($vd_id, $sndate, "OUTPUT", "01", $refnum,
		"Non-Stock invoice No. $inv[invnum] Credit note No.$noteid Customer $inv[cusname].", $TOTAL*-1, $inv["vat"]*-1);
