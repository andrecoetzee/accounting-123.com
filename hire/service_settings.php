...
function save()
{
	...
	foreach ($days as $asset_id => $days) {
		...
		if (!pg_numrows($svdate_rslt)) {
			$sql = "
				INSERT INTO cubit.asset_svdates (
					asset_id, svdate, des
				) VALUES (
					'$asset_id', '$svdate', '$description[$asset_id]'
				)";
		} else {
			$sv_data = pg_fetch_array($svdate_rslt);

			$sql = "
				UPDATE cubit.asset_svdates 
				SET svdate='$svdate', description='$description[$asset_id]' 
				WHERE id='$sv_data[id]'";
	...
	$message = "
		<li style='background: ".TMPL_tblDataColor1."; width: 100%'>
