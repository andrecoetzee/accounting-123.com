...
function details($_POST, $error="")
{
	...
	$hiddens.= "<input type='hidden' name='accountss[0]' value='0' />";
	$hiddens.= "<input type='hidden' name='descriptionss[0]' value='0' />";
	$hiddens.= "<input type='hidden' name='vatcodess[0]' value='0' />";
	$hiddens.= "<input type='hidden' name='whidss[0]' value='2' />";
	$hiddens.= "<input type='hidden' name='qtyss[0]' value='1' />";
	$hiddens.= "<input type='hidden' name='discs[0]' value='0' />";
	$hiddens.= "<input type='hidden' name='discps[0]' value='0' />";
...
/* --- Start Drop Downs --- */
	...
	if ($_POST["key"] == "recvpayment_write") {
			...
			if(isset($stkidss[$key]) && $stkidss[$key] != "-S" && (strlen($stkidss[$key]) > 0)){
				...
				if(!isset($diffwhBtn)){
					...
					if(!isset($addnon)) {
						...
						if(isset($des)and $des!="") {$len=strlen($des);$Wh="AND lower(substr(stkdes,1,'$len'))=lower('$des')";} else {$Wh="";$des="";}
...
/* --- End Some calculations --- */
	...
	if($inv['rounding']>0) {
		...
		$rd="<tr bgcolor='".TMPL_tblDataColor1."'><td>Rounding</td><td align=right>R $inv[rounding]</td></tr>
		<tr bgcolor='".TMPL_tblDataColor2."'><th>Amount Due</th><td align=right>R $due</td></tr>";
		...
		if ($line_count > 0) {
			...
			<tr bgcolor='".TMPL_tblDataColor1."'>
				<td colspan='2' align='center'>
					<input type='submit' name='recvpay' id='recvpay' onclick='return ptot_recvpay();'
						value='Receive Payment: ".CUR." ".sprint($inv["pcc"] + $inv["pcheque"] + $inv["pcash"])."' />
				</td>
			</tr>";
		}
	}
	...
	<table cellpadding='".TMPL_tblCellPadding."' cellspacing='".TMPL_tblCellSpacing."' border='0' width=95%>
 	<tr><td valign='top'>
		<table cellpadding='".TMPL_tblCellPadding."' cellspacing='".TMPL_tblCellSpacing."' border='0'>
			...
			<tr bgcolor='".TMPL_tblDataColor1."'><td>Department</td><td valign='center'>$depts</td></tr>
...
<!--			<tr bgcolor='".TMPL_tblDataColor2."'><td>Customer Order number</td><td valign='center'><input type='text' size='10' name='cordno' value='$cordno'></td></tr>
			...
			<tr bgcolor='".TMPL_tblDataColor1."'><td>Search for description</td><td><input type='text' size='13' name='des' value='$des'></td></tr>
			<tr bgcolor='".TMPL_tblDataColor2."'><td colspan='2' align='center'><input type='submit' value='Search'></td></tr>
			<tr><th colspan='2'>Options</th></tr>
			<tr bgcolor='".TMPL_tblDataColor1."'><td>Select Using</td><td>Stock Code<input type='radio' name='sel_frm' value='stkcod' onChange='javascript:document.form.submit();' $sel_frm_cod> Stock Description<input type='radio' name='sel_frm' value='stkdes' onChange='javascript:document.form.submit();' $sel_frm_des></td>
-->
		</table>
...
<!--			<tr>
			...
			<tr bgcolor='".TMPL_tblDataColor1."'><td>Invoice No.</td><td valign='center'>$inv[invid]</td></tr>
			<tr bgcolor='".TMPL_tblDataColor2."'><td>Sales Order No.</td><td valign='center'><input type='text' size='5' name='ordno' value='$inv[ordno]'></td></tr>
			<tr bgcolor='".TMPL_tblDataColor1."'>
				<td>Invoice Date</td>
			...
			<tr bgcolor='".TMPL_tblDataColor2."'><td>Sales Person</td>$sales</tr>
			...
			<tr bgcolor='".TMPL_tblDataColor1."'><td>Delivery Charge</td><td valign='center'><input type='text' size='7' name='delchrg' value='$inv[delchrg]'>$Vatcodes</td></tr>
-->
...
<!--	<tr><td><br></td></tr>
	...
	<tr><td align='right'><input name='diffwhBtn' type='submit' value='Different Store'> | <input name='addprodBtn' type='submit' value='Add Product'>| <input name='addnon' type='submit' value='Add Non stock Product'>$save </td><td>| <input type='submit' name='upBtn' value='Update'>$done</td></tr>
	</table><a name='bottom'>
-->
	</form></center>";
...
/* --- Start button Listeners --- */
	if(isset($doneBtn)){
		...
		# print the invoice
		$OUTPUT = "<script>printer2('pos-invoice-print.php?invid=$invid');move('pos-invoice-new.php');</script>";
		require("template.php");
		...
		$write = "
		<table border='0' cellpadding='".TMPL_tblCellPadding."' cellspacing='".TMPL_tblCellSpacing."'>
			<tr><th>New Point of Sale Invoice Saved</th></tr>
			<tr bgcolor='".TMPL_tblDataColor2."'><td>Invoice for <b>$client</b> has been saved.</td></tr>
		</table>
		<p>
		<table border='0' cellpadding='".TMPL_tblCellPadding."' cellspacing='".TMPL_tblCellSpacing."'>
	...
	if (isset($invids)) {
		foreach($invids as $key => $value) {
			...
			if (!isset($itype[$ii]) && !isset($ptype[$ii])) {
				$sql = "SELECT prd,invnum FROM cubit.invoices
				...
				$sql = "INSERT INTO cubit.stmnt(cusnum, invid, amount, date, type, div)
						VALUES('$cus[cusnum]','$inv[invnum]',
							...
							'Payment for Invoice No. $inv[invnum]', '".USER_DIV."')";
			...
			/* OPTION 1: NONS STOCK INVOICES */
				...
				$sql = "INSERT INTO cubit.stmnt(cusnum, invid, amount, date, type, div)
						VALUES('$cus[cusnum]','$inv[invnum]',
						'".($paidamt[$key] - ($paidamt[$key] * 2))."','$sdate',
						'Payment for Non Stock Invoice No. $inv[invnum]',
						'".USER_DIV."')";
				$stmntRslt = db_exec($sql) or errDie("Unable to insert statement record in Cubit.",SELF);
